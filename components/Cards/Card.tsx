...
import { getImagePath } from "@/lib/utils/tmdbUtils";
...
function Card({ imageSlug, description, backdropSlug, onLoad }: Props) {
  ...
  function onLoadingComplete(img: HTMLImageElement) {
    setIsLoading(false);
  ...
  return (
    <div
      ref={cardRef}
      className="h-[600px] w-96 flex transition-all ease-in-out rounded-xl overflow-hidden relative"
    >
      {!isLoading && (
        <>
          <div
            className={classNames(
              "absolute left-0 z-10 flex w-1/3 h-full bg-gradient-fade-left opacity-0 transition-opacity duration-300",
              curPg !== 1 && "hover:opacity-30"
            )}
          />
          <div
            className={classNames(
              "absolute right-0 z-10 flex w-1/3 h-full bg-gradient-fade-right opacity-0 transition-opacity duration-300",
              curPg !== 3 && "hover:opacity-30"
            )}
          />
        </>
      )}
...
function LoadingPage({ visible = false }) {
  return (
    <div
      className="flex w-full h-full z-10 absolute bg-black"
      style={{ visibility: visible ? "visible" : "hidden" }}
    >
      <Triangle
        color="rgba(226, 43, 255, 1)"
        height={200}
        width={200}
        wrapperClass="w-full flex items-center justify-center rotate-180"
      />
    </div>
  );
}
  ...
  return (
    <>
      ...
      >
        ...
        {imageSlug && (
          <Image
            src={getImagePath(imageSlug, "w500")}
            fill
            draggable="false"
            alt=""
